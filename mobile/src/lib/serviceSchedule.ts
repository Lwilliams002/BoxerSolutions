...
/** Short suffix for totals: "$89/mo", "$40/2 wks". */
export const SERVICE_FREQUENCY_SHORT: Record<ServiceFrequency, string> = {
  weekly: '/wk',
  biweekly: '/2 wks',
  monthly: '/mo',
  bimonthly: '/2 mo',
...
/** Column label for a schedule cell: "Sep '26" for month cadences, "Sep 8" for week cadences. */
export function scheduleCellLabel(dateIso: string, frequency: ServiceFrequency): string {
  const [y, m, d] = dateIso.split('-').map(Number);
  const date = new Date(y, m - 1, d, 12);
  if (frequency === 'monthly' || frequency === 'bimonthly' || frequency === 'quarterly') {
